...
export class EditDetailComponent implements OnChanges {
  ...
  selectCategName(Id: number) {
    if (Id) {
      this.QuesDetailsCopy.CategoryName = this.CategoriesV.find(
        ele => ele.Id == Id
      ).Name;
    }
  }
  ...
  Save() {
    ...
    q["Category"] = this.CategoriesV.find(
      ele => ele.Id == this.QuesDetails.CategoryId
    );
    ...
    this.srv.putQues(q).subscribe(res => {
      ...
      this.QuesDetails.Answers.forEach(ele => {
        this.srv.putAnswer(ele).subscribe(a_res => {
          console.log(`Answer with id #${a_res.Id} has been updated .`);
        });
      });
    });
  }
  ...
  ngOnChanges() {
    ...
    this.srv.getQuesDetails(this.ques.Id).subscribe(
      res => {
        this.QuesDetails = res;
      },
      err => alert(" connection faild " + err.statusText)
    );
    this.editable = false;
  }
}
